...
# 3 "fpga/kernel.cpp" 2
# 14 "fpga/kernel.cpp"
void initialization(double *f ){

    VITIS_LOOP_16_1: for (int iy = 0; iy < 16; iy++) {
        VITIS_LOOP_17_2: for (int ix = 0; ix < 16; ix++) {
          const double x = 2.0 * ix / (16 - 1.0) - 1.0;
          const double y = 2.0 * iy / (16 - 1.0) - 1.0;
...
void creating(double *v) {
 VITIS_LOOP_27_1: for (int iy = 0; iy < 16; iy++) {
         VITIS_LOOP_28_2: for (int ix = 0; ix < 16; ix++) {
          v[16*iy + ix] = 0.0;
         }
 }
}
...
__attribute__((sdx_kernel("kernel", 0))) void kernel(double *v , bool *convFPGA, unsigned int *numIter){
#line 19 "C:/Users/Leonardo/Documents/GitHub/MdP-Poisson2DJacobi/fpga/solution1/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=kernel
# 36 "fpga/kernel.cpp"

#line 7 "C:/Users/Leonardo/Documents/GitHub/MdP-Poisson2DJacobi/fpga/solution1/directives.tcl"
#pragma HLSDIRECTIVE TOP name=kernel
# 36 "fpga/kernel.cpp"


#pragma HLS INTERFACE m_axi depth=16*16 port=v bundle=gmem0
...
#pragma HLS INTERFACE s_axilite port=return
 ...
 VITIS_LOOP_65_1: while ((e > 1e-5) && (n < 200000))
 {
  e = 0.0;

  VITIS_LOOP_69_2: for( int ix = 1; ix < (16 -1); ix++ )
  {
   VITIS_LOOP_71_3: for (int iy = 1; iy < (16 -1); iy++)
   {
    double d;

    vp[iy*16 +ix] = -0.25 * (f[iy*16 +ix] -
     (v[16*iy + ix+1] + v[16*iy + ix-1] +
      v[16*(iy+1) + ix ] + v[16*(iy-1) + ix ]));

    d = fabs(vp[16*iy+ix] - v[16*iy+ix]);
    e = (d > e) ? d : e;
   }
  }
  ...
  VITIS_LOOP_88_4: for (int ix = 1; ix < (16 -1); ix++)
  {
   VITIS_LOOP_90_5: for (int iy = 1; iy < (16 -1); iy++)
   {
    v[16*iy+ix] = vp[16*iy+ix];
    w += fabs(v[16*iy+ix]);
   }
  }

  VITIS_LOOP_97_6: for (int ix = 1; ix < (16 -1); ix++)
  {
   v[16*0 + ix] = v[16*(16 -2) + ix];
   v[16*(16 -1) + ix] = v[16*1 + ix];
   w += fabs(v[16*0+ix]) + fabs(v[16*(16 -1)+ix]);
  }

  VITIS_LOOP_104_7: for (int iy = 1; iy < (16 -1); iy++)
  {
   v[16*iy + 0] = v[16*iy + (16 -2)];
   v[16*iy + (16 -1)] = v[16*iy + 1 ];
   w += fabs(v[16*iy+0]) + fabs(v[16*iy+(16 -1)]);
  }
